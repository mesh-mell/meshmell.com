...
const ModelPage = ({ userId, modelId }: ModelPageType) => {
  ...
  useEffect(() => {
    ...
    const getModelByUserIdAndModelId = async (userId: string, modelId: string) => {
      onValue(usersRef, (snapshot) => {
        ...
        if (user) {
          // Find the model
          const model = user.models.find((m: ShareModelType) => m.model_id === modelId);
  ...
  return (
    <>
      <OrbitControls
        ...
        maxDistance={50}
      />
      <LightForScene lightAndDarkTheme={theme} />
      <mesh
        ref={meshRef}
        castShadow
        receiveShadow
